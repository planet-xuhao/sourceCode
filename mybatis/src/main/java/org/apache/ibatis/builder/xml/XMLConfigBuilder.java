...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  private XMLConfigBuilder(XPathParser parser, String environment, Properties props) {
    //首先调用父类初始化Configuration，为父类成员变量赋值,注册了各种别名
...
//  </configuration>
    // 根节点是configuration
    // 解析配置，从configuration根节点开始解析，最终给configuration成员变量赋值
  ...
  private void parseConfiguration(XNode root) {
    try {
      ...
      // 10.映射器，解析mappers节点，mapper节点会告诉mybatis去哪些位置找到mapper的***Mapper.xml文件，以及使用了注解标识的接口
